...
class GrlNode :public Node {
  ...
  public:
    ...
    void backward() {
        //in->loss().vec() += loss().vec() * in->val().vec() * grl_val;
        in->loss().vec() += loss().vec() * grl_val;
    }
...
class GrlExecutor :public Executor {
public:
...
PExecutor GrlNode::generate() {
    GrlExecutor* exec = new GrlExecutor();
    exec->batch.push_back(this);
    return exec;
};
