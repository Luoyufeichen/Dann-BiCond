...
public:
    ...
    void forward(Graph &cg, vector<Node *>& x, Node& guide) {
        ...
        for (int idx = 0; idx < x.size(); idx++) {
            ...
            uni_node->init(1);
            uni_node->setNodeName("attention");
...
struct SelfAttentionParams : public N3LDGSerializable
{
    UniParams uni_atten;
    int hidden_dim;
    ...
    Json::Value toJson() const override {
        Json::Value json;
        json["uni_atten"] = uni_atten.toJson();
        json["hidden_dim"] = hidden_dim;
        return json;
    }

    void fromJson(const Json::Value &json) override {
        uni_atten.fromJson(json["uni_atten"]);
        hidden_dim = json["hidden_dim"].asInt();
    }
...
public:
    ...
    void init(SelfAttentionParams* paramInit, int maxsize) {
