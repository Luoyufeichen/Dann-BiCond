...
dtype softMaxLoss(PNode x, const string &answer, Metric& eval, int batchsize = 1) {
		int nDim = x->getDim();
		int labelsize = 3;
		if (labelsize != nDim) {
			std::cerr << "softmax_loss error: dim size invalid" << nDim << std::endl;
			return -1.0;
		}
...
dtype softMaxLoss_target(PNode x, const string &answer, Metric& eval, int batchsize = 1, dtype targetLoss = 0.81) {
		int nDim = x->getDim();
		int labelsize = 4;
		if (labelsize != nDim) {
			std::cerr << "softmax_loss for target error: dim size invalid  " << nDim <<  std::endl;
			abort();
                        //return -1.0;
		}
		...
		NRVec<dtype> scores(nDim);
		int ans;
		if (answer == "atheism") ans = 0;
		else if (answer == "climate") ans = 1;
		else if (answer == "feminist") ans = 2;
		else if (answer == "abortion") ans = 3;
                else{
                    cout << "no such target:  " << answer <<endl;
                    abort();
                }
		dtype cost = 0.0;
		int optLabel = -1;
		...
		for (int i = 0; i < nDim; ++i) {
			float t = ans == i ? 1.0 : 0.0;
			x->loss()[i] = targetLoss * (scores[i] / sum2 - t) / batchsize;
		}
...
dtype softMaxLoss_binary(PNode x, const string &answer, Metric& eval, int batchsize = 1, dtype targetLoss = 0.81) {
		int nDim = x->getDim();
		int labelsize = 2;
		if (labelsize != nDim) {
			std::cerr << "softmax_loss for target2 error: dim size invalid  " << nDim <<  std::endl;
			abort();
                        //return -1.0;
		}
...
void predictLoss(PNode x, int& y, bool bTargetInTweet) {
		int nDim = x->getDim();
		int optLabel = -1;
		for (int i = 0; i < nDim; ++i) {
			if (bTargetInTweet && i == 2) continue;
