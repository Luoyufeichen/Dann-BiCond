...
public:
	...
	void setOptions(const vector<string> &vecOption) {
		...
		for (; i < vecOption.size(); ++i) {
			...
			if (pr.first == "featCutOff")
				targetCutOff = atoi(pr.second.c_str());
			...
			if (pr.first == "grl")
				grl = atof(pr.second.c_str());
			if (pr.first == "targetLoss")
				targetLoss = atof(pr.second.c_str());
	...
	void showOptions() {
		...
		std::cout << "featCutOff = " << targetCutOff << std::endl;
		...
		std::cout << "grl = " << grl << std::endl;
		std::cout << "targetLoss = " << targetLoss << std::endl;
